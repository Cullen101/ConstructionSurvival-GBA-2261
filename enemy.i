...
void initEnemy() {
    for (int i = 0; i < 4; i++) {
        ...
        enemy[i].col = 240 / 2 - enemy[i].width * 2;
        if (i % 2 == 0) {
            enemy[i].active = 1;
        } else {
            enemy[i].active = 0;
...
void updateEnemy(ENEMY *e) {
    if (e->active == 1) {
        ...
        if(e->aniCounter % 15 == 0) {
            ...
            if (collisionmapBitmap[((e->row) * (256) + (e->col - e->cdel))] && collisionmapBitmap[((e->row + e->height - 1) * (256) + (e->col - e->cdel))] &&
            collisionmapBitmap[((e->row) * (256) + (e->col + e->width - 1 + e->cdel))] && collisionmapBitmap[((e->row + e->height - 1) * (256) + (e->col + e->width - 1 + e->cdel))]) {
                if (e->cdel == -1) {
        ...
        if (e->row + e->height < 256 && collisionmapBitmap[((e->row + e->height - 1 + 2) * (256) + (e->col))] && collisionmapBitmap[((e->row + e->height - 1 + 2) * (256) + (e->col + e->width - 1))]) {
            e->row += e->rdel;
        }

        if (e->row + e->height > 250) {
            e->row = 0 + e->height - 8;
            e->col = 240 / 2 - e->width * 2;
        }

        if (collision(e->col, e->row, e->width, e->height,
            player.col, player.row, player.width, player.height)) {
                    e->active = 0;
                    shadowOAM[5 + e->value].attr0 = (2 << 8);
                    e->cdel = -(e->cdel);
...
void drawEnemy(ENEMY *e) {
    ...
    if (e->active) {
        shadowOAM[5 + e->value].attr0 = (0xFF & (e->row - vOff)) | (0 << 13) | (0 << 14);
        shadowOAM[5 + e->value].attr1 = (0x1FF & (e->col - hOff)) | (1 << 14);
        shadowOAM[5 + e->value].attr2 = ((e->curFrame * 2)*32 + (e->aniState * 2)) | ((0) << 12);
    }
}
